...
export const FirestoreDashboardPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'users' | 'settings' | 'content'>('overview');
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        console.warn('[FirestoreDashboardPage] Failed to fetch remote users, using localStorage fallback.');
      }
  ...
  const handleRoleChange = async (email: string, currentRole: string) => {
    ...
    try {
      ...
      console.warn('[FirestoreDashboardPage] Direct Firestore/API role update failed, updating local state.');
    }
  ...
  return (
    <div className="w-full bg-[var(--bg-surface)] border border-[var(--border-color)] rounded-3xl shadow-[0_25px_70px_rgba(0,240,255,0.15)] overflow-hidden flex flex-col min-h-[80vh] pb-6 animate-in fade-in duration-350">
      ...
      <div className="px-6 py-5 border-b border-[var(--border-color)] bg-[var(--bg-surface-solid)]/90 flex items-center justify-between">
        <div className="flex items-center space-x-3.5">
          <div className="p-2.5 bg-[var(--color-cyan)]/10 border border-[var(--color-cyan)]/35 rounded-2xl text-[var(--color-cyan)] shadow-[0_0_15px_rgba(0,240,255,0.2)]">
          ...
          <div>
            <h2 className="text-lg font-extrabold text-white font-heading tracking-wide flex items-center gap-2">
              ...
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-[var(--color-cyan)]/20 text-[var(--color-cyan)] border border-[var(--color-cyan)]/30 uppercase tracking-widest font-mono font-bold">
                PAGE
              </span>
            </h2>
            <p className="text-xs text-[var(--text-muted)]">Real-time platform governance, moderation, and content management</p>
          </div>
        </div>
        <div className="flex items-center space-x-2 text-xs font-mono px-3 py-1.5 bg-[var(--bg-surface-solid)] border border-[var(--border-color)] rounded-xl text-emerald-450">
        ...
        {[
          ...
          return (
            <button
              ...
              className={`flex items-center space-x-2 px-5 py-4 text-xs font-bold transition-all border-b-2 whitespace-nowrap ${
                isActive
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="p-5 bg-[var(--bg-surface-solid)] border border-[var(--border-color)] rounded-2xl relative overflow-hidden">
                ...
                <Users className="absolute right-3 bottom-3 w-10 h-10 text-[var(--color-cyan)]/10" />
              </div>
              ...
              <div className="p-5 bg-[var(--bg-surface-solid)] border border-[var(--border-color)] rounded-2xl relative overflow-hidden">
                ...
                <Bot className="absolute right-3 bottom-3 w-10 h-10 text-purple-500/10" />
              </div>
              ...
              <div className="p-5 bg-[var(--bg-surface-solid)] border border-[var(--border-color)] rounded-2xl relative overflow-hidden">
                ...
                <FileText className="absolute right-3 bottom-3 w-10 h-10 text-emerald-500/10" />
              </div>
            </div>

            {/* System Health */}
            <div className="p-6 bg-[var(--bg-surface-solid)] border border-[var(--border-color)] rounded-2xl space-y-3">
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3 bg-[var(--bg-surface-solid)] border border-[var(--border-color)] rounded-xl text-xs text-white placeholder-gray-500 focus:outline-none focus:border-[var(--color-cyan)]/50"
              />
            </div>
              ...
              {filteredUsers.length === 0 ? (
                ...
                filteredUsers.map((u) => {
                  ...
                  return (
                    <div
                      key={u.email}
                      className="flex items-center justify-between p-3.5 bg-[var(--bg-surface-solid)]/60 border border-[var(--border-color)] rounded-2xl hover:border-[var(--color-cyan)]/25 transition-all"
                    >
      ...
      {/* Footer */}
      <div className="px-6 py-4 bg-[var(--bg-surface-solid)]/90 border-t border-[var(--border-color)] flex items-center justify-between text-[10px] text-[var(--text-muted)]">
        <div className="flex items-center space-x-2">
          <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />
