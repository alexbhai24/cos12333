import React, { useState, useMemo, useEffect } from 'react';
import { useSyllabusStorage } from '../hooks/useSyllabusStorage';
import { 
  getSchoolBoards, 
  getClasses, 
  getStreams, 
  getSpecialExams, 
  resolveSyllabusId, 
  getSyllabusData 
} from '../data/syllabusDatabase';
import { ChapterAccordion } from '../components/syllabus/ChapterAccordion';
import { ChapterCardView } from '../components/syllabus/ChapterCardView';
import { ChapterDetailView } from '../components/syllabus/ChapterDetailView';
import { 
  Search, GraduationCap, Flame, LayoutGrid, ListTodo, ChevronDown, RotateCcw, BookOpen, Check, Plus
} from 'lucide-react';
import { BackgroundCanvas } from '../components/BackgroundCanvas';
import { TopBar } from '../components/TopBar';
import { Sidebar } from '../components/Sidebar';
import { useApp } from '../context/AppContext';

type ViewMode = 'CARDS' | 'LIST';
type CategoryMode = 'School' | 'Special';

export const SyllabusTrackerPage: React.FC = () => {
  const { sidebarCollapsed } = useApp();

  // Primary States
  const [categoryMode, setCategoryMode] = useState<CategoryMode>('School');
  const [viewMode, setViewMode] = useState<ViewMode>('CARDS');
  const [searchQuery, setSearchQuery] = useState('');

  // Filter States
  const [selectedBoard, setSelectedBoard] = useState('CBSE');
  const [selectedClass, setSelectedClass] = useState('Class 12');
  const [selectedStream, setSelectedStream] = useState('Commerce');
  const [selectedSpecialExam, setSelectedSpecialExam] = useState('neet');
  
  // Active Data State
  const [activeSubjectId, setActiveSubjectId] = useState<string | null>(null);
  const [selectedChapterId, setSelectedChapterId] = useState<string | null>(null);

  // Compute the exact syllabus ID based on current filters
  const resolvedId = resolveSyllabusId(
    categoryMode,
    selectedSpecialExam,
    selectedBoard,
    selectedClass,
    selectedStream
  );

  const syllabus = getSyllabusData(resolvedId);
  const { topicState, toggleTopic, resetSubjectProgress, getChapterProgress, getSubjectProgress, getOverallProgress } = useSyllabusStorage(resolvedId || '');

  const validSubjects = useMemo(() => {
    return (syllabus?.subjects || []).filter(Boolean);
  }, [syllabus]);

  // Ensure an active subject is selected when syllabus loads
  useEffect(() => {
    if (validSubjects.length > 0) {
      if (!activeSubjectId || !validSubjects.find(s => s.id === activeSubjectId)) {
        setActiveSubjectId(validSubjects[0].id);
      }
    } else {
      setActiveSubjectId(null);
    }
  }, [validSubjects, activeSubjectId]);

  // Derived Active Data
  const activeSubject = validSubjects.find(s => s.id === activeSubjectId);
  const activeSubjectProgress = activeSubject ? getSubjectProgress(activeSubject) : { percentage: 0, completedTopics: 0, totalTopics: 0 };
  
  // Calculate how many chapters are 100% complete
  const completedChaptersCount = useMemo(() => {
    if (!activeSubject || !Array.isArray(activeSubject.chapters)) return 0;
    return activeSubject.chapters.filter(ch => getChapterProgress(ch).isCompleted).length;
  }, [activeSubject, getChapterProgress]);

  // Search Filtering Logic
  const filteredChapters = useMemo(() => {
    if (!activeSubject || !Array.isArray(activeSubject.chapters)) return [];
    if (!searchQuery.trim()) return activeSubject.chapters;
    
    const lowerQ = searchQuery.toLowerCase();
    return activeSubject.chapters.filter(ch => {
      const matchTitle = (ch.title || '').toLowerCase().includes(lowerQ);
      const matchTopic = (ch.topics || []).some(t => (t.title || '').toLowerCase().includes(lowerQ));
      return matchTitle || matchTopic;
    });
  }, [activeSubject, searchQuery]);

  // Selected Chapter for Full Detail View
  const selectedChapter = useMemo(() => {
    if (!selectedChapterId || !activeSubject) return null;
    return (activeSubject.chapters || []).find(c => c.id === selectedChapterId) || null;
  }, [selectedChapterId, activeSubject]);

  const selectedChapterIndex = useMemo(() => {
    if (!selectedChapter || !activeSubject) return 0;
    return (activeSubject.chapters || []).findIndex(c => c.id === selectedChapter.id);
  }, [selectedChapter, activeSubject]);

  // Reset chapter selection when subject or syllabus changes
  useEffect(() => {
    setSelectedChapterId(null);
  }, [resolvedId, activeSubjectId]);

  // If a chapter is selected, render the dedicated Chapter Detail Page
  if (selectedChapter) {
    return (
      <div className="animate-in fade-in duration-300 pb-16">
        <ChapterDetailView
          chapter={selectedChapter}
          chapterIndex={selectedChapterIndex}
          subjectName={(activeSubject?.name || '').replace(/\s*\(Theory:.*?\)/gi, '').replace(/\s*\(.*Marks\)/gi, '').trim()}
          examContext={`${selectedBoard} ${selectedClass} ${selectedStream || ''}`}
          progress={getChapterProgress(selectedChapter)}
          topicState={topicState}
          onToggleTopic={toggleTopic}
          onBack={() => setSelectedChapterId(null)}
        />
      </div>
    );
  }

  return (
    <div className="space-y-6 animate-in fade-in duration-300 pb-16">
      {/* Top Control Bar (Search + Modes) */}
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-6">
        
        {/* Global Search */}
        <div className="relative flex-1 max-w-xl">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
          <input 
            type="text"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search chapters, topics, formulas, or marking scheme..."
            className="w-full bg-[var(--bg-surface)] border border-white/10 rounded-2xl py-3 pl-12 pr-4 text-sm text-white placeholder:text-gray-500 focus:outline-none focus:border-[var(--color-cyan)] shadow-lg"
          />
        </div>

        {/* Category Mode Toggles */}
        <div className="flex items-center gap-3">
          <button
            onClick={() => setCategoryMode('School')}
            className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-bold transition-all ${
              categoryMode === 'School' 
              ? 'bg-[var(--color-cyan)] text-black shadow-[0_0_20px_rgba(0,240,255,0.3)]' 
              : 'bg-[var(--bg-surface)] border border-white/10 text-gray-400 hover:text-white'
            }`}
          >
            <GraduationCap className="w-4 h-4" />
            School Boards (Class 6-12)
          </button>
          
          <button
            onClick={() => setCategoryMode('Special')}
            className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-bold transition-all ${
              categoryMode === 'Special' 
              ? 'bg-rose-500 text-white shadow-[0_0_20px_rgba(244,63,94,0.3)]' 
              : 'bg-[var(--bg-surface)] border border-white/10 text-gray-400 hover:text-white'
            }`}
          >
            <Flame className="w-4 h-4" />
            NEET & JEE Main (Special)
          </button>
        </div>
      </div>

      {/* Secondary Filter Bar */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
        
        {/* Cascading Dropdowns */}
        <div className="flex flex-wrap items-center gap-3">
          {categoryMode === 'School' ? (
            <>
              <div className="flex items-center gap-2 bg-[var(--bg-surface)] border border-white/10 rounded-xl px-3 py-1.5">
                <span className="text-xs text-gray-500 font-bold">Board:</span>
                <select 
                  value={selectedBoard}
                  onChange={e => setSelectedBoard(e.target.value)}
                  className="bg-transparent text-sm font-bold text-white outline-none cursor-pointer"
                >
                  {getSchoolBoards().map(b => <option key={b} value={b}>{b}</option>)}
                </select>
              </div>
              
              <div className="flex items-center gap-2 bg-[var(--bg-surface)] border border-white/10 rounded-xl px-3 py-1.5">
                <span className="text-xs text-gray-500 font-bold">Class:</span>
                <select 
                  value={selectedClass}
                  onChange={e => setSelectedClass(e.target.value)}
                  className="bg-transparent text-sm font-bold text-white outline-none cursor-pointer"
                >
                  {getClasses().map(c => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>

              {/* Show Stream only for 11 & 12 */}
              {['Class 11', 'Class 12'].includes(selectedClass) && (
                <div className="flex items-center gap-2 bg-[var(--bg-surface)] border border-white/10 rounded-xl px-3 py-1.5">
                  <span className="text-xs text-gray-500 font-bold">Stream:</span>
                  <select 
                    value={selectedStream}
                    onChange={e => setSelectedStream(e.target.value)}
                    className="bg-transparent text-sm font-bold text-[var(--color-cyan)] outline-none cursor-pointer"
                  >
                    {getStreams().map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                </div>
              )}
            </>
          ) : (
            <div className="flex items-center gap-2 bg-[var(--bg-surface)] border border-white/10 rounded-xl px-3 py-1.5">
              <span className="text-xs text-gray-500 font-bold">Exam:</span>
              <select 
                value={selectedSpecialExam}
                onChange={e => setSelectedSpecialExam(e.target.value)}
                className="bg-transparent text-sm font-bold text-rose-400 outline-none cursor-pointer"
              >
                {getSpecialExams().map(ex => <option key={ex.id} value={ex.id}>{ex.name}</option>)}
              </select>
            </div>
          )}
        </div>

        {/* View Toggle */}
        <div className="flex items-center bg-[var(--bg-surface)] border border-white/10 rounded-xl p-1 shrink-0">
          <button
            onClick={() => setViewMode('CARDS')}
            className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${
              viewMode === 'CARDS' ? 'bg-[var(--color-cyan)] text-black' : 'text-gray-400 hover:text-white'
            }`}
          >
            <LayoutGrid className="w-3.5 h-3.5" />
            Cards
          </button>
          <button
            onClick={() => setViewMode('LIST')}
            className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${
              viewMode === 'LIST' ? 'bg-[var(--color-cyan)] text-black' : 'text-gray-400 hover:text-white'
            }`}
          >
            <ListTodo className="w-3.5 h-3.5" />
            To-Do List
          </button>
        </div>
      </div>

      {/* Subject Pills Row */}
      <div className="flex items-center gap-3 overflow-x-auto custom-scrollbar pb-2 mb-6 border-b border-white/5">
        {validSubjects.length > 0 ? (
          validSubjects.map(subject => {
            const cleanName = subject.name.replace(/\s*\(Theory:.*?\)/gi, '').replace(/\s*\(.*Marks\)/gi, '').trim();
            return (
              <button
                key={subject.id}
                onClick={() => setActiveSubjectId(subject.id)}
                className={`shrink-0 px-5 py-2 rounded-full text-sm font-bold transition-all border ${
                  activeSubjectId === subject.id 
                  ? 'bg-[var(--color-cyan)]/10 text-[var(--color-cyan)] border-[var(--color-cyan)]' 
                  : 'bg-[var(--bg-surface)] text-gray-400 border-white/10 hover:border-white/30 hover:text-white'
                }`}
              >
                {cleanName}
              </button>
            );
          })
        ) : (
          <span className="text-sm text-gray-500 italic px-2">No subjects found for this combination...</span>
        )}
      </div>

      {/* Sticky Progress Bar */}
      {activeSubject && (
        <div className="sticky top-[72px] z-20 mb-5 pt-2">
          <div className="relative overflow-hidden bg-transparent border border-white/10 backdrop-blur-xl rounded-2xl px-4 py-3 shadow-none">
            {/* Background glowing ambient light */}
            <div 
              className="absolute -top-10 -right-10 w-36 h-36 bg-emerald-500 rounded-full blur-[70px] opacity-15 pointer-events-none transition-opacity duration-1000 animate-pulse-slow"
              style={{ opacity: activeSubjectProgress.percentage > 0 ? 0.25 : 0.05 }}
            />
            
            {/* Header info row */}
            <div className="flex items-center justify-between gap-3 mb-2.5 relative z-10">
              <div className="flex flex-col gap-0.5 min-w-0">
                <div className="flex items-center gap-2">
                  <h3 className="text-sm sm:text-base font-extrabold text-white truncate">
                    {completedChaptersCount} of {activeSubject.chapters.length} chapters completed
                  </h3>
                  {activeSubjectProgress.percentage === 100 && (
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-black bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 shadow-[0_0_10px_rgba(16,185,129,0.3)]">
                      COMPLETED
                    </span>
                  )}
                </div>
                <span className="text-[11px] sm:text-xs text-gray-400">
                  {activeSubjectProgress.completedTopics} of {activeSubjectProgress.totalTopics} topics ticked off
                </span>
              </div>
              
              <button 
                onClick={() => resetSubjectProgress(activeSubject)}
                title="Reset Subject Progress"
                className="flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-bold text-white/70 bg-white/10 hover:bg-white/20 hover:text-white border border-white/15 transition-all active:scale-95 group shadow-sm shrink-0"
              >
                <RotateCcw className="w-3 h-3 group-hover:-rotate-180 transition-transform duration-500 ease-out" />
                Reset
              </button>
            </div>
            
            {/* Main Progress Row: Compact Stepped Capsule Bar on Left + Compact Percentage Circle on Right */}
            <div className="flex items-center gap-3 sm:gap-4 relative z-10">
              
              {/* Stepped Capsule Bar */}
              <div className="flex-1 flex flex-col gap-1 min-w-0">
                <div className="relative h-3 sm:h-8 md:h-9 w-full bg-white/10 border border-white/15 rounded-full shadow-inner overflow-hidden p-0.5 sm:p-1 flex items-center">
                  
                  {/* Dynamic Gradient Fill */}
                  <div 
                    className="absolute top-0 bottom-0 left-0 rounded-full transition-all duration-700 ease-out bg-gradient-to-r from-[#059669] via-[#10b981] via-[#34d399] to-[#6ee7b7]"
                    style={{ 
                      width: activeSubjectProgress.percentage <= 0 ? '0%' : `${Math.max(activeSubjectProgress.percentage, 4)}%`,
                      boxShadow: activeSubjectProgress.percentage > 0 ? '0 0 15px rgba(16, 185, 129, 0.4)' : 'none'
                    }}
                  />

                  {/* Chapter Milestone Circles along the Track (Hidden on mobile screens to prevent crowding) */}
                  <div className="relative w-full h-full hidden sm:flex items-center justify-between px-2 sm:px-3 z-10">
                    {activeSubject.chapters.map((ch, idx) => {
                      const chProg = getChapterProgress(ch);
                      const isDone = chProg.isCompleted;
                      const isPartiallyDone = !isDone && chProg.completedTopics > 0;
                      const isFirst = idx === 0;
                      const isLast = idx === activeSubject.chapters.length - 1;
                      const totalChapters = activeSubject.chapters.length;

                      // Color progression for completed chapters
                      let completedBgClass = 'bg-[#10b981] text-white shadow-sm';
                      if (isFirst) {
                        completedBgClass = 'bg-white text-emerald-800 shadow-[0_1px_6px_rgba(0,0,0,0.35)]';
                      } else if (isLast) {
                        completedBgClass = 'bg-[#10b981] text-white shadow-[0_0_10px_rgba(16,185,129,0.7)]';
                      } else {
                        const ratio = idx / (totalChapters - 1);
                        if (ratio <= 0.3) completedBgClass = 'bg-[#0f764e] text-white';
                        else if (ratio <= 0.6) completedBgClass = 'bg-[#34d399] text-[#064e3b]';
                        else completedBgClass = 'bg-[#6ee7b7] text-[#064e3b]';
                      }

                      return (
                        <div 
                          key={ch.id} 
                          title={`Ch ${idx + 1}: ${ch.title} (${chProg.completedTopics}/${ch.topics.length} topics)`}
                          className="flex items-center justify-center cursor-pointer transition-transform hover:scale-110"
                        >
                          <div 
                            className={`w-5 h-5 sm:w-6 sm:h-6 rounded-full flex items-center justify-center transition-all duration-500 shadow-sm ${
                              isDone 
                                ? completedBgClass
                                : isLast
                                ? 'bg-white/20 text-white/70 backdrop-blur-sm'
                                : isPartiallyDone
                                ? 'bg-emerald-500/25 text-emerald-300 border border-emerald-500/40 animate-pulse'
                                : 'bg-white/10 text-white/40 border border-white/15'
                            }`}
                          >
                            {isLast && !isDone ? (
                              <Plus className="w-2.5 h-2.5 sm:w-3 sm:h-3 stroke-[2.5]" />
                            ) : (
                              <Check className={`w-2.5 h-2.5 sm:w-3 sm:h-3 stroke-[3] ${!isDone && !isPartiallyDone ? 'opacity-30' : 'opacity-100'}`} />
                            )}
                          </div>
                        </div>
                      );
                    })}
                  </div>
                </div>

                {/* Chapter Numbers Under Capsule (Hidden on mobile screens) */}
                <div className="w-full hidden sm:flex items-center justify-between px-3 sm:px-4">
                  {activeSubject.chapters.map((ch, idx) => {
                    const isDone = getChapterProgress(ch).isCompleted;
                    const isFirst = idx === 0;
                    return (
                      <div key={`num-${ch.id}`} className="flex items-center justify-center w-5 sm:w-6">
                        {isFirst && isDone ? (
                          <span className="w-3.5 h-3.5 rounded-full bg-emerald-500 text-black text-[9px] font-black flex items-center justify-center shadow-[0_0_6px_rgba(16,185,129,0.5)]">
                            1
                          </span>
                        ) : (
                          <span className={`text-[10px] sm:text-[11px] font-bold transition-colors ${
                            isDone ? 'text-emerald-400' : 'text-gray-500/60'
                          }`}>
                            {idx + 1}
                          </span>
                        )}
                      </div>
                    );
                  })}
                </div>
              </div>

              {/* Right Side: Compact Radial Percentage Circle */}
              <div className="shrink-0 flex items-center justify-center">
                <div className="relative w-11 h-11 sm:w-13 sm:h-13 flex items-center justify-center">
                  <svg className="w-full h-full -rotate-90" viewBox="0 0 44 44">
                    <defs>
                      <linearGradient id="circleProgressGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                        <stop offset="0%" stopColor="#047857" />
                        <stop offset="50%" stopColor="#10b981" />
                        <stop offset="100%" stopColor="#34d399" />
                      </linearGradient>
                    </defs>
                    {/* Background Track Circle */}
                    <circle
                      cx="22"
                      cy="22"
                      r="17"
                      fill="transparent"
                      stroke="rgba(255, 255, 255, 0.15)"
                      strokeWidth="3.5"
                    />
                    {/* Progress Circle Fill */}
                    <circle
                      cx="22"
                      cy="22"
                      r="17"
                      fill="transparent"
                      stroke="url(#circleProgressGradient)"
                      strokeWidth="3.5"
                      strokeDasharray={106.81}
                      strokeDashoffset={106.81 - (activeSubjectProgress.percentage / 100) * 106.81}
                      strokeLinecap="round"
                      className="transition-all duration-700 ease-out"
                      style={{
                        filter: activeSubjectProgress.percentage > 0 ? 'drop-shadow(0 0 5px rgba(16,185,129,0.5))' : 'none'
                      }}
                    />
                  </svg>
                  {/* Inside Circle: Percentage Calculated */}
                  <div className="absolute inset-0 flex flex-col items-center justify-center text-center pointer-events-none">
                    <span className="text-[10px] sm:text-xs font-black tracking-tight text-white drop-shadow-sm">
                      {Math.round(activeSubjectProgress.percentage)}%
                    </span>
                  </div>
                </div>
              </div>

            </div>
          </div>
        </div>
      )}

      {/* Content Area */}
      {!syllabus ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <div className="w-20 h-20 bg-white/5 rounded-full flex items-center justify-center mb-4 border border-white/10">
            <Search className="w-8 h-8 text-gray-500" />
          </div>
          <h2 className="text-xl font-bold text-white mb-2">Syllabus Data Not Available</h2>
          <p className="text-gray-400 max-w-md">
            We haven't indexed the official syllabus for this specific combination yet. Check back soon or try another class!
          </p>
        </div>
      ) : (
        <div className="relative">
          {filteredChapters.length === 0 ? (
            <div className="text-center py-12 text-gray-400">
              No chapters match your search query.
            </div>
          ) : viewMode === 'CARDS' ? (
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 pb-20">
              {filteredChapters.map((chapter, idx) => (
                <ChapterCardView
                  key={chapter.id}
                  index={idx}
                  chapter={chapter}
                  progress={getChapterProgress(chapter)}
                  topicState={topicState}
                  onToggleTopic={toggleTopic}
                  onSelectChapter={(id) => setSelectedChapterId(id)}
                  subjectName={(activeSubject?.name || '').replace(/\s*\(Theory:.*?\)/gi, '').replace(/\s*\(.*Marks\)/gi, '').trim()}
                  exam={categoryMode === 'Special' ? (selectedSpecialExam === 'neet' ? 'NEET' : 'JEE') : undefined}
                  isSpecialExam={categoryMode === 'Special'}
                />
              ))}
            </div>
          ) : (
            <div className="max-w-4xl mx-auto space-y-3 pb-20">
              {filteredChapters.map(chapter => (
                <ChapterAccordion
                  key={chapter.id}
                  chapter={chapter}
                  progress={getChapterProgress(chapter)}
                  topicState={topicState}
                  onToggleTopic={toggleTopic}
                  onSelectChapter={(id) => setSelectedChapterId(id)}
                />
              ))}
            </div>
          )}
        </div>
      )}
    </div>
  );
};
