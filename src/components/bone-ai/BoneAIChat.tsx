...
export const BoneAIChat: React.FC<BoneAIChatProps> = ({ 
  ...
  useEffect(() => {
    if (textareaRef.current) {
      ...
      const targetHeight = Math.min(Math.max(scrollHeight, 38), 240);
    ...
    try {
      ...
      if (response.error) {
        ...
        onUpdateMessage(botMsgId, {
          ...
          isVerified: response.isVerified || isCheckMode,
        ...
        {showVoiceMode ? (
          ...
          >
            ...
            <textarea
              ...
              className="w-full bg-transparent border-0 text-white text-[13.5px] placeholder:text-gray-500 focus:outline-none resize-none min-h-[38px] max-h-[240px] px-1 leading-relaxed scrollbar-premium"
              rows={1}
            />
